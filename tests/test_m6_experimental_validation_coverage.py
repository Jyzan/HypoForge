...
def _module(payload):
    return ExperimentalValidationAuditor(
        client=FakeValidationClient(payload),
        llm_config=None,
        timeout_seconds=1.0,
    )
...
def test_validation_targets_cover_all_m4_sections():
    targets = ExperimentalValidationAuditor.validation_targets(_hypothesis())
...
async def test_complete_m5_plan_covers_every_target():
    ...
    verdict = (await module.audit(_hypothesis(), _plan(), 1)).verdict

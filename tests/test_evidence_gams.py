...
def test_evidence_gams_removes_low_quality_and_incoherent_edges() -> None:
    candidates = [
        ...
        candidate("BAD_CONFLICT", "C1", "C4", "contradicts", 0.18, [], "P1", "P4"),
        candidate("BAD_UNSOURCED", "C1", "C2", "supports", 0.25, [], "P1", "P2"),
    ]
    ...
    assert "BAD_CONFLICT" not in selected_ids
    assert "BAD_UNSOURCED" not in selected_ids
    assert {"R1", "R2", "R3", "R5"}.issubset(selected_ids)
    assert all(item.evidence_ids and item.confidence >= 0.35 for item in selected)


def test_evidence_gams_is_reproducible_with_fixed_seed() -> None:
    candidates = [
        candidate("R1", "C1", "C2", "supports", 0.9, ["E1"], "P1", "P2"),
        candidate("R2", "C2", "C3", "extends", 0.8, ["E2"], "P2", "P3"),
        candidate("R3", "C1", "C3", "limits", 0.7, ["E3"], "P1", "P3"),
    ]
    first, first_trace = EvidenceGraphGAMS(random_seed=7).search(candidates, iterations=64)
    second, second_trace = EvidenceGraphGAMS(random_seed=7).search(candidates, iterations=64)

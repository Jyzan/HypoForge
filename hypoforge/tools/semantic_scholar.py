...
def _http_get_json(url: str) -> dict:
    ...
    for attempt in range(4):
        ...
        if _S2_API_KEY and "semanticscholar.org" in url:
            req.add_header("x-api-key", _S2_API_KEY)
        try:
...
def _s2_search(query: str, limit: int) -> List[dict]:
...
def _s2_fetch(paper_id: str) -> dict:
    ...
    return _s2_normalise(_http_get_json(url))
...
def _oa_search(query: str, limit: int) -> List[dict]:
    """Search OpenAlex, return standardised paper dicts."""
    url = (
        f"{_OA_BASE}/works?"
        f"search={urllib.parse.quote(query)}"
        f"&per_page={min(limit, 200)}"
...
def _oa_fetch(work_id: str) -> dict:
...
def _fetch(identifier: str) -> dict:
    ...
    if _BACKEND == "semantic_scholar":
        return _s2_fetch(identifier)
    else:
        return _oa_fetch(identifier)
...
class SemanticScholarTool(ToolProtocol):
    ...
    async def search(self, query: str, limit: int = 20, **kwargs) -> List[dict]:
        ...
        """
        try:
            return _search(query, limit=min(max(limit, 1), 200))
        except Exception:

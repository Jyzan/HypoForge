...
Ported from feature/m3-evidence-gams.  Experimental — default OFF.
"""
...
class EvidenceGraphGAMS:
        ...
        for index, left in enumerate(selected):
            for right in selected[index + 1:]:
                ...
                if relations == {"supports", "contradicts"}:
                    comparability = max(
                        left.condition_comparability,
                        right.condition_comparability,
                    )
                    penalty += 0.16 + 0.18 * comparability
                elif (
                    left.relation != right.relation
                    and relations <= {"same_as", "contradicts", "refines"}
                ):
                    penalty += 0.10
    ...
    def score(
        ...
        confidence = sum(
            c.confidence for c in selected
        ) / len(selected)
        possible_nodes = {
            ep
            for candidate in candidates
            for ep in (candidate.source, candidate.target)
        }
        covered_nodes = {
            ep
        ...
        provenance = sum(
            bool(candidate.evidence_ids) for candidate in selected
        ) / len(selected)
        cross_source = sum(
            bool(candidate.source_paper_ids)
            and bool(candidate.target_paper_ids)
            and set(candidate.source_paper_ids).isdisjoint(
                candidate.target_paper_ids
            )
            for candidate in selected
        ) / len(selected)
        conditional = [
        ...
        comparability = (
            sum(conditional) / len(conditional) if conditional else 1.0
        )
        retrieval_quality = sum(
            candidate.retrieval_score for candidate in selected
        ) / len(selected)
        credible_indices = {
            index
        ...
        credible_edge_recall = (
            len(selected_indices & credible_indices)
            / max(len(credible_indices), 1)
        )
        ...
        source_dependence_penalty = sum(
            bool(candidate.source_paper_ids)
            and bool(candidate.target_paper_ids)
            and not set(candidate.source_paper_ids).isdisjoint(
                candidate.target_paper_ids
            )
            for candidate in selected
        ) / len(selected) * 0.08
    ...
    def _register(
        ...
        if indices not in states:
            ...
            states[indices] = _StateStats(
                reward=reward, components=components
            )
        return states[indices]

    def _select_state(
        self, states: Dict[FrozenSet[int], _StateStats]
    ) -> FrozenSet[int]:
        ...
        def uct(item: tuple[FrozenSet[int], _StateStats]) -> float:
            ...
            exploration = self.exploration_weight * math.sqrt(
                math.log(total_visits + 1) / stat.visits
            )
    ...
    def _best_addition(
        ...
        return max(
            ...
            key=lambda state: self._register(
                states, candidates, frozenset(state)
            ).reward,
        )
    ...
    def _intra_branch_evolution(
        ...
        for index in current:
            ...
            options.append(
                frozenset(self._best_addition(states, candidates, reduced))
            )
        return max(
            options,
            key=lambda state: self._register(
    ...
    def _cross_branch_reference(
        ...
        donors = [
            state
            for state in self._elite_states(states)
            if state != current and state - current
        ]
        if not donors:
        ...
        return frozenset(
            self._best_addition(
                states, candidates, current, donor - current
            )
        )
    ...
    def _prune_union(
        ...
        while improved and current:
            improved = False
            current_reward = self._register(
                states, candidates, current
            ).reward
            options = [
                frozenset(set(current) - {index}) for index in current
            ]
            best = max(
                options,
                key=lambda state: self._register(
            ...
            if (
                self._register(states, candidates, best).reward
                > current_reward + 1e-9
            ):
                current = best
                improved = True
    ...
    def _multi_branch_aggregation(
        ...
        return self._prune_union(
            states, candidates, frozenset(current | partner)
        )
    ...
    def _rollout(
        ...
        for _ in range(self.rollout_depth):
            ...
            proposal = frozenset(
                self._best_addition(states, candidates, state, sample)
            )
            if (
                self._register(states, candidates, proposal).reward + 1e-9
                < self._register(states, candidates, state).reward
            ):
                break
            state = proposal
        return state
    ...
    def search(
        ...
        operator_counts = {op: 0 for op in self.OPERATORS}
        operations = {
        ...
        for iteration in range(max(1, iterations)):
            ...
            child = self._rollout(
                states, searchable, frozenset(child)
            )
        ...
        best_state, best_stat = max(
            states.items(), key=lambda item: item[1].reward
        )
        ...
        trace: Dict[str, object] = {
            ...
            "best_components": {
                key: round(value, 6)
                for key, value in best_stat.components.items()
            },
            "direct_accept_all_reward": round(direct_stat.reward, 6),
            "direct_accept_all_components": {
                key: round(value, 6)
                for key, value in direct_stat.components.items()
            },
